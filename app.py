...
def main():
    ...
    uploaded_zip = st.file_uploader(
        "📁 Téléchargez le fichier ZIP du dossier de consultation",
        type=["zip"],
        help="Fichiers volumineux acceptés. La barre de progression s'affiche après réception."
    )

    if uploaded_zip is None:
        st.info("⏳ En attente du fichier ZIP… Les fichiers volumineux peuvent prendre quelques secondes à charger.")
        return
    ...
    # ── Step 0 : Save + extract ZIP ───────────────────────────
    st.markdown("---")
    st.subheader("📦 Étape 1 : Chargement et extraction")

    file_size_mb = uploaded_zip.size / (1024 * 1024)
    st.info(f"📁 Fichier reçu : **{uploaded_zip.name}** — {file_size_mb:.1f} MB")

    with st.status("📤 Chargement du fichier en cours…", expanded=True) as upload_status:

        # ── Read file bytes with chunked progress bar ─────────
        st.write("💾 Lecture du fichier…")
        progress_bar = st.progress(0, text="Chargement… 0%")

        file_bytes  = uploaded_zip.getbuffer()
        total_size  = len(file_bytes)
        chunk_size  = max(total_size // 40, 1024)   # ~40 steps, min 1 KB
        ...
        with open(temp_zip, "wb") as f:
            ...
            for i in range(0, total_size, chunk_size):
                ...
                pct = min(int(bytes_written / total_size * 100), 100)
                progress_bar.progress(pct / 100, text=f"Chargement… {pct}%")

        progress_bar.progress(1.0, text="✅ Fichier chargé !")
        st.write(f"✅ Fichier sauvegardé ({file_size_mb:.1f} MB)")

        # ── Extract ZIP ───────────────────────────────────────
        st.write("📦 Extraction du ZIP…")
        extract_zip(temp_zip, extract_root)
        temp_zip.unlink()
        st.write("✅ Extraction terminée.")

        # ── Find RC / CCTP / DPGF files ───────────────────────
        st.write("🔍 Recherche des fichiers CCTP / DPGF / RC…")
        ...
        if not rc_files:
            ...
            upload_status.update(label="❌ Aucun RC trouvé.", state="error")
            return

        rc_path = rc_files[0]
        st.write(f"✅ Fichier RC trouvé : **{rc_path.name}**")
        ...
        upload_status.update(label="✅ Chargement et extraction terminés !", state="complete")

    st.success(f"✅ Fichier RC prêt : {rc_path.name}")
